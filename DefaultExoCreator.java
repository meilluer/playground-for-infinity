...
public class DefaultExoCreator implements ExoCreator, MediaSourceEventListener {
    ...
    public DefaultExoCreator(@NonNull ToroExo toro, @NonNull Config config) {
        ...
        SharedPreferences sharedPreferences = PreferenceManager.getDefaultSharedPreferences(toro.context);
        ConnectivityManager connectivityManager = (ConnectivityManager) toro.context.getSystemService(Context.CONNECTIVITY_SERVICE);
        String videoQualityKey = "video_quality_wifi";
        if (connectivityManager != null) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                NetworkCapabilities networkCapabilities = connectivityManager.getNetworkCapabilities(connectivityManager.getActiveNetwork());
                if (networkCapabilities != null) {
                    if (networkCapabilities.hasTransport(NetworkCapabilities.TRANSPORT_CELLULAR)) {
                        videoQualityKey = "video_quality_mobile";
                    }
                }
            } else {
                // Use deprecated method for older versions
                if (connectivityManager.getActiveNetworkInfo() != null && connectivityManager.getActiveNetworkInfo().getType() == ConnectivityManager.TYPE_MOBILE) {
                    videoQualityKey = "video_quality_mobile";
                }
            }
        }

        String videoQuality = sharedPreferences.getString(videoQualityKey, "auto");
        DefaultTrackSelector.ParametersBuilder parametersBuilder = new DefaultTrackSelector.ParametersBuilder(toro.context);
        if (!"auto".equals(videoQuality)) {
            try {
                int maxVideoHeight = Integer.parseInt(videoQuality);
                parametersBuilder.setMaxVideoSize(Integer.MAX_VALUE, maxVideoHeight);
            } catch (NumberFormatException e) {
                // "auto" or invalid value, do nothing
            }
        }
        trackSelector = new DefaultTrackSelector(toro.context, parametersBuilder.build());
        ...
        if (baseFactory == null) {
            baseFactory = new DefaultHttpDataSource.Factory().setAllowCrossProtocolRedirects(true).setUserAgent(APIUtils.sUserAgent);
        }
